<div class="form-group mb-4">
    ...
    <input type="text" name="name" required value="{{$role->name}}" class="form-control input-sm" placeholder='Name' />
</div>
<div class="form-group mb-4">
    ...
    <select name="permission[]" id="permissions" class="form-control input-sm" multiple="">
        <?php foreach($permission as $permission){ ?>
                <option value="{{$permission->id}}" <?php echo in_array($permission->id,$rolePermissions)? 'selected="selected"':''; ?>>{{$permission->name}}</option>
        <?php } ?>
    </select>
</div>
